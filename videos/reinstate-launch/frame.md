...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Questrial for all
  display + Questrial body + Geist Mono Variable for the index/code voice on a warm-navy code surface,
...
typography:
  ...
  card-title:{ fontFamily: "Geist", cqw: 2.3, weight: 550, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "Geist", cqw: 1.46, weight: 550, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "Geist", cqw: 1.35, weight: 550, tracking: "0.18em", upper: true }
  kicker:  { fontFamily: "Geist Mono Variable", px: 28, cqw: 1.46, weight: 500, tracking: "0.16em", upper: true }
  mono-label:{ fontFamily: "Geist Mono Variable", px: 26, cqw: 1.35, weight: 500, tracking: "0.02em" }
  code:    { fontFamily: "Geist Mono Variable", cqw: 1.67, weight: 400, lineHeight: 1.6 }
  # — display ramp (Questrial 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "Questrial", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "Questrial", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "Questrial", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "Questrial", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "Questrial", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "Questrial", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
  number-unit:{ fontFamily: "Geist Mono Variable", cqw: 2.08, weight: 500, lineHeight: 1.0 }
...
components:
  ...
  kicker-spike:
    ...
    description: "The eyebrow — Geist Mono Variable uppercase, indexical (2–5 words), prefixed with the coral ✱. Never plain text, never a sentence."
  coral-callout:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Questrial figure paired with a Geist Mono Variable unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Questrial italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Questrial italic, small Questrial uppercase cite beneath."
  section-rule:
  ...
  code-surface:
    ...
    textColor: "{colors.cream} (Geist Mono Variable); syntax in coral (keywords) / teal #5DB8A6 (strings) / amber #E8A55A (numbers)"
...
- **Fonts** — already set to **Questrial** (display) / **Geist Variable** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 500, 540, 550, 560, 600, 650, 700}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
Three editorial voices, each in its own face: **Questrial** carries every display moment — covers,
...
UI chrome. **Geist Mono Variable** carries the indexical layer — kickers, technical labels, the code
window, status strips. Switching a voice's face collapses the register: a Geist headline or a
Questrial label reads as a different brand.

**In this video the mono voice is not decoration — it is the product.** Every
command, path, session id, and status line renders in Geist Mono Variable, and
those frames are the proof beats. Never substitute the display or body face
inside a terminal surface.
...
- **Questrial** (sentence case, negative-tracked) for all display; **Geist** body/chrome; **Geist Mono Variable** index + code.
...
- **Squint** — one Questrial display moment dominates at 3–6× its neighbor.
...
- **Type** — Questrial sentence-case display (negative-tracked); Questrial 400 body; Geist Mono Variable kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (Questrial `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Questrial `headline` 4.6cqw
...
- **Questrial display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Questrial body** sentence case weight 400. **Geist Mono Variable** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Questrial figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Questrial italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Questrial · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Questrial `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Questrial 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Questrial line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Questrial `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a Questrial `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: Questrial italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a Questrial **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small Questrial uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** Questrial italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short Questrial sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **Questrial, sentence case**, negative-tracked; **Questrial 400** body; **Geist Mono Variable** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Questrial figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Questrial display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Questrial display moment dominates.
...
- **Type** — Questrial sentence-case display (negative-tracked); Questrial 400 body; Geist Mono Variable kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
...
**Three faces, staged from the live site's own WOFF2** — Questrial (display),
Geist (body + UI chrome), Geist Mono Variable (the terminal / index voice).
`build-frame.mjs` stages only the display face; Geist and Geist Mono were added
by hand because the mono voice is load-bearing in this video — every command,
path, and session id renders in it, and a fallback mono would break the
terminal illusion. Both are variable faces, so any weight 100–900 is available.
