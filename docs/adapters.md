...
Claude Code and Codex CLI are T5. OpenCode is T4. Gemini CLI and Grok Build
are T2.
...
| OpenCode | Read-only in `v0.2.0` | Yes (T3) | Yes | Yes (T4) | No | Later |
...
Handoff support is directional. Claude Code, Codex and OpenCode are both
sources and targets. Gemini CLI and Grok Build are source-only: their transcript
