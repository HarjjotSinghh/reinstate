...
Claude Code, Codex CLI, and OpenCode are T5 — OpenCode is the first
embedded-SQLite agent to reach encrypted sync, with the physical round-trip
recorded on macOS and native Windows. Qwen Code and Grok Build are T4.
Gemini CLI and Kimi Code CLI are T2. Grok's resume and destination journeys
are recorded on both platforms. The capability matrix below remains the
fail-closed per-surface record.
...
| OpenCode | Read-only in `v0.2.0` | Yes (T3) | Source-only | Yes | Supported (T5) | Later |
