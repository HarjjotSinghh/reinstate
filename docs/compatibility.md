...
| Qwen Code | Read-only included | Same-vendor native resume and fork | Dual-platform probes committed; macOS and native Windows T3/T4 journeys committed; verified range widened to 0.21.12-0.23.0 on native Windows evidence (macOS pending) | T4 |
...
ceiling to `1.18.29` (`1.18.27` at first, widened again to `1.18.29` after the
acceptance host self-updated past that ceiling), both **widened on native
Windows evidence; macOS pending** (ADR 0005 D2/D3 — the macOS host is in
repair, so this release ships on the Windows-first acceptance waiver). On
...
measured on a device through `v0.5.1`; `v0.6.0` widens it to `1.18.21`–`1.18.29`
