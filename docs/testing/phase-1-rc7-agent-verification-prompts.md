# Phase 1 RC7 autonomous two-agent acceptance prompts

Use these prompts only after `v0.1.0-rc.7` is published and both public
installers install that exact release. Do not use RC5 or older homes, profiles,
passphrases, marker sessions, or reports as RC7 evidence.

The
[Phase 1 MacBook + Windows acceptance runbook](phase-1-mac-windows-acceptance.md)
from the signed RC7 tag remains authoritative for product behavior, mandatory
...
REINSTATE_ENCRYPTION_PASSPHRASE=<dedicated RC7 test passphrase>
```
...
the fresh RC7 disposable project:

```text
Run the complete Device A side of Reinstate Phase 1 acceptance against the
published v0.1.0-rc.7. You are the macOS evidence owner and cross-device
...
- Fetch origin and the signed tag v0.1.0-rc.7.
...
- The only allowed repository change is the sanitized report:
  docs/testing/results/REPORT_DATE-macos-phase1-rc7.md
- Resolve REPORT_DATE yourself from the current UTC date in YYYY-MM-DD form;
  the human does not edit the prompt or filename.
- Use branch test/phase1-rc7-macos-report from the peeled RC7 tag commit.
...
- Never use --all. Operate only on the two fresh RC7 marker session IDs.
...
Isolation:
- REINSTATE_HOME=$HOME/.reinstate-phase1-acceptance-rc7
- project=$HOME/Projects/reinstate-phase1-acceptance-rc7
...
1. Verify v0.1.0-rc.7 is an annotated signed tag reachable from origin/main.
   Record its commit without changing trust configuration to force a pass.
2. Verify the live install.sh returns 200, pins only rc.7, verifies both
   checksum layers, installs 0.1.0-rc.7 without elevation, and is idempotent
...
7. Run first-device init non-interactively with the canonical mapping. Use
   ...
   Record the generated non-secret fresh RC7 profile_id.
...
profile_id=<non-secret fresh rc7 uuid>
...
mac_report_path=docs/testing/results/REPORT_DATE-macos-phase1-rc7.md
END-MAC-RC7-M1
...
Milestone M2 — after a WINDOWS-RC7-W1-PASS report:
...
14. Autonomously resume the exact Claude session on Mac, add only
    REINSTATE-PHASE1-RC7-MAC-CLAUDE-A2, exit it, then dry-run and push only that
    ID. Update/commit/push the Mac report with MAC-RC7-M2-READY containing the
    session ID and new non-secret snapshot/revision IDs. Pause for the report
    transfer to Windows.

Milestone M3 — after a WINDOWS-RC7-W2-READY report:
...
16. Prove each existing target received a timestamped backup. Resume each exact
    ...
    - REINSTATE-PHASE1-RC7-WINDOWS-CLAUDE-B1
    - REINSTATE-PHASE1-RC7-WINDOWS-CODEX-B1
...
18. Update/commit/push the report with MAC-RC7-M3-PASS containing sanitized
...
19. After Windows reports its unpushed local divergence ready, resume the exact
    Mac Claude ID, add only REINSTATE-PHASE1-RC7-CONFLICT-MAC, exit, and push
...
20. Consume the final Windows report. Reconcile every mandatory section 19 row
    across both reports. Independently query the RC7 publication PR/check run
...
published v0.1.0-rc.7. You are the native-Windows evidence owner. Work
...
Repository scope:
- The only allowed repository change is the sanitized report:
  docs/testing/results/REPORT_DATE-windows-phase1-rc7.md
- Resolve REPORT_DATE yourself from the current UTC date in YYYY-MM-DD form;
  the human does not edit the prompt or filename.
- Use branch test/phase1-rc7-windows-report from the peeled RC7 tag commit.
- At each handoff, commit only that report, push the existing branch, and create
  or update one draft PR. Never merge, tag, release, deploy, or modify a product
  branch.
- Add R2.txt to .git/info/exclude before any report-repository mutation. Prove
...
Required Mac report:
- It contains MAC-RC7-M1 for v0.1.0-rc.7, the fresh profile/session IDs,
  canonical project ID local/reinstate-phase1-acceptance-rc7, exactly two
...
1. Verify v0.1.0-rc.7 is an annotated signed tag reachable from origin/main.
2. Verify live install.ps1 returns 200, pins only rc.7, verifies both checksum
   layers, installs 0.1.0-rc.7 without elevation, is idempotent, and produces
...
codex_resume=PASS
windows_report_path=docs/testing/results/REPORT_DATE-windows-phase1-rc7.md
END-WINDOWS-RC7-W1

On failure, use WINDOWS-RC7-W1-FAIL with exact command, exit, and sanitized
...
Milestone W2 — after a MAC-RC7-M2-READY report:
...
17. Close Claude autonomously, dry-run and pull the same ID, prove a timestamped
    ...
    REINSTATE-PHASE1-RC7-MAC-CLAUDE-A2.
18. Resume the exact Windows Claude and Codex sessions, add only:
    - REINSTATE-PHASE1-RC7-WINDOWS-CLAUDE-B1
    - REINSTATE-PHASE1-RC7-WINDOWS-CODEX-B1
    Exit both, dry-run, and push only those IDs.
19. Update/commit/push the report with WINDOWS-RC7-W2-READY containing
...
20. After MAC-RC7-M3-PASS, resume the exact Windows Claude session, add only
    REINSTATE-PHASE1-RC7-CONFLICT-WINDOWS, exit, and do not push. Update the
    report with WINDOWS-RC7-CONFLICT-LOCAL-READY and pause for transfer to Mac.
...
23. Reconcile all mandatory section 19 rows with the latest Mac report.
    Independently query the RC7 publication PR/check run and verify every
...
1. Mac produces `MAC-RC7-M1`; human passes the Mac report to Windows.
2. Windows produces `WINDOWS-RC7-W1-PASS`; human passes the Windows report to Mac.
3. Mac produces `MAC-RC7-M2-READY`; human passes the Mac report to Windows.
4. Windows produces `WINDOWS-RC7-W2-READY`; human passes the Windows report to Mac.
5. Mac produces `MAC-RC7-M3-PASS`; human passes the Mac report to Windows.
6. Windows produces `WINDOWS-RC7-CONFLICT-LOCAL-READY`; human passes the Windows
   report to Mac.
