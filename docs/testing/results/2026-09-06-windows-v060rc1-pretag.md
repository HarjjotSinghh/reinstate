...
**This block is recomputed as of the second pass on `86cb3421`
(2026-09-06) — see [Second pass, 86cb3421 (2026-09-06)](#second-pass-86cb3421-2026-09-06)
for the full evidence. First-pass figures (against `57c15d52`) are kept
alongside as history, not deleted, per the same append-only convention
"Verification round 2" used above.**

- **Device verdict:** `FAIL` — **16** of the 200 rows this report is
  ...
  *(First pass: 43 of 200 did not pass.)* Named row-by-row in
...
- **Milestone:** `MATRIX_COMPLETE` for the 178 Phase 5 rows and 22 CLI
  ...
  `RECONCILED`; 6 rows remain genuinely `NOT TESTED` and are not claimed
  as evidenced. See the gaps below and
...
- **Required counts — Phase 5 generated matrix (178 rows), latest result:**
  `162 PASS / 9 FAIL / 1 PARTIAL / 6 NOT TESTED`.
  *(First pass: `137 PASS / 2 FAIL / 1 PARTIAL / 38 NOT TESTED`.)*
- **Required counts — CLI experience (22 rows), latest result:**
  `22 PASS / 0 FAIL / 0 PARTIAL / 0 NOT TESTED`.
  *(First pass: `20 PASS / 2 FAIL / 0 PARTIAL / 0 NOT TESTED`.)*
- **Required counts — combined (200 rows), latest result:**
  `184 PASS / 9 FAIL / 1 PARTIAL / 6 NOT TESTED`.
  *(First pass: `157 PASS / 4 FAIL / 1 PARTIAL / 38 NOT TESTED`.)*
...
- **Automated gates (contract section A, 8 rows, tracked separately from
  ...
  latest result:** `8 PASS / 0 FAIL` — `A7` re-run clean end to end
  (coordinator's run, and this pass's own from-scratch 4-step rebuild).
  *(First pass: `7 PASS / 1 FAIL` (`A7`, a staging-directory gap).)*
- **Release-blocking findings, latest status:** `RB1` (dead spacebar),
  `RB5` (`A7` staging), and `RB6` (row-22 baseline drift) are **resolved**
  by the second pass — their rows now `PASS` and no fresh row failure
  reopens them. `RB2` (`opencode` `D1` `NOT_INSTALLED`) is **substantially
  de-risked** — `D1:opencode` now `PASS`es against a real vendor session
  in 10 of 11 attempts, with the one failure root-caused to a narrow,
  load-dependent defect tracked as `PD-1` (not release-blocking on this
  evidence). `RB4` (the T4/T5 physical-resume gap) is **partially
  resolved and narrowed**: `codex`, `opencode`, and `grok` now have full
  real physical resume/fork/version-boundary evidence; `claude`'s gap
  turned into a **new, distinct, confirmed release-blocking finding**
  (`RB8` below — the shipped version range already trails the vendor's
  real shipped build, so `claude` native resume/fork never launches on
  this host as configured); `qwen`'s `E2`/`E3` content-continuity proof
  remains blocked by an expired host-side credential unrelated to
  Reinstate; the T5 encrypted-sync round trip remains entirely
  `NOT TESTED`. The second pass also surfaced one **new confirmed
  release-blocking product defect** not in the first pass's list —
  `RB7`, `agent.active` silently reporting a confident "not active"
  instead of "unknown" when Windows process enumeration fails, defeating
  its own documented fail-safe design; this reproduced for all 5 required
  T4/T5 agents this pass. See
