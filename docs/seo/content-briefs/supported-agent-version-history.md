...
- Capabilities: Claude Code `2.1.219`–`2.1.228` and Codex CLI `0.133.0`–`0.147.0` are the current inclusive source-tested stable ranges; out-of-range or prerelease versions fail closed as untested. Claude Code `2.1.228` entered the range with the unreleased `v0.4.0-rc.1` candidate and still awaits dual-platform tagged-artifact acceptance; stable `v0.3.0` shipped with the ceiling at `2.1.227`.
