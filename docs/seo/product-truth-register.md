...
Reviewed release: `v0.4.0-rc.5` candidate (tagged for testing; tagged-artifact
...
| Cross-agent behavior | `v0.4.0-rc.5` candidate code provides explicit structured handoff into a new Claude Code or Codex session; it does not translate or transfer a native session | handoff contract, ADR 0003, CLI/doctest contracts |
...
| Current release | `v0.4.0-rc.5` candidate, acceptance pending; `v0.3.0` remains stable | changelog, release history, compatibility data |
...
- Gemini CLI, OpenCode, or Grok Build as `v0.4.0-rc.5` handoff destinations;
...
1. `v0.4.0-rc.5` still requires signed tagged-artifact acceptance on Apple
