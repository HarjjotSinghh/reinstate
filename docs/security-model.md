...
| Keyring | `<prefix>/keyring.v1.json` in storage | Per key generation, the root key wrapped to each enrolled device's public key (age) and under the recovery code (argon2id → XChaCha20-Poly1305). Format version 4. Every wrap is bound to the profile id and the generation it belongs to (device wraps inside the age payload, the recovery wrap as AEAD associated data), so a wrap cannot be replayed in another keyring or generation; and every generation, the first included, carries a `signature` — ed25519 over its own header, under a keypair derived from the recovery code — so only a holder of the code can write one. The public half is published as `account_key`, so any device verifies without holding a key. Versions 1, 2 and 3 are not read at all: 1 and 2 tied nothing between generations, and 3 keyed the tie to the previous generation's root key, which is exactly what the device being revoked was holding. Written create-only on init and with compare-and-swap on every later change; a write that would take the object past three quarters of the 1 MiB a read accepts is refused, because the object only ever grows. |
...
| `account.json` | Reinstate home | Format version 2: device id, the account signing key's public half, the key generation this device last unwrapped and that generation's root-key **recipient** (all three public, and together the anchor the keyring is checked against), enrolment route, and a local-only boolean recording that the recovery code was confirmed on this device. No key material; all three anchor fields are required. |
