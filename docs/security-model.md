...
| Keyring | `<prefix>/keyring.v1.json` in storage | Per key generation, the root key wrapped to each enrolled device's public key (age) and under the recovery code (argon2id → XChaCha20-Poly1305). Format version 3. Every wrap is bound to the profile id and the generation it belongs to (device wraps inside the age payload, the recovery wrap as AEAD associated data), so a wrap cannot be replayed in another keyring or generation; and every generation past the first carries a `chain` — HMAC-SHA256 over its own header, keyed by the previous generation's root key — so only a holder of that key can write it. Versions 1 and 2 are not read at all: they had no chain, and reading them would leave that hole open. Written create-only on init and with compare-and-swap on every later change; a write that would take the object past three quarters of the 1 MiB a read accepts is refused, because the object only ever grows. |
...
| `account.json` | Reinstate home | Device id, key generation, that generation's root-key **recipient** (public, and the anchor the generation chain hangs from), enrolment route, and a local-only boolean recording that the recovery code was confirmed on this device. No key material. |
...
code), draws a fresh root key, wraps it for every remaining device's public
key and under the code, and appends the generation to the keyring with
compare-and-swap; earlier generations are never modified. Every object stays
...
That window is long enough to **write** the keyring, so the keyring
authenticates itself in two layers.

The first is the chain. Generation N+1 carries an HMAC-SHA256 over its own
header — number, `created_at`, `recipient`, the revocations that started it,
the profile id, and the number and recipient of generation N — keyed by a
value derived (HKDF-SHA256, its own info string) from **generation N's root
key**. A device revoked at generation N never held N+1's key and cannot
obtain N's again, so it cannot produce a generation the account will adopt.
Every command that acts on the keyring verifies the chain against the root
keys it just unwrapped, and refuses a current generation it cannot trace
back to one it already trusted: push, pull, `devices approve`, `devices
revoke`, `account recover`. The device wraps inside a generation are
deliberately not covered by that generation's chain, because devices are
enrolled into generations after the fact; they do not need to be, since a
wrap is only accepted when the key inside it derives the generation's
recorded `recipient`, which the chain does cover. Appending a *working*
wrap therefore still requires that generation's root key.

The second is the anchor, because the chain proves only a relative fact. A
keyring forged from generation 1 upward under a root key of the attacker's
choosing is self-consistent, and nothing inside the object could tell it
from the account's own. So each device records, in `account.json`, the
generation it last unwrapped and that generation's root-key recipient
(public, not key material). A keyring whose current generation is lower is
a rollback; one where the recorded generation is missing or now names a
different root key was replaced rather than appended to. Both fail closed
with nothing written, on every path, including `account status` and `rein
devices`, which hold no root key and check the anchor alone. A device with
no local record anchors on what brought it there: the recovery code, whose
holder is the only party that can write a recovery wrap that opens, or the
root key an enrolled device relayed through a pairing approval and which
the joining device matches against the keyring's own wrap for it.

Taken together: against a party that can read and write the locker bucket,
no remaining device seals anything to a root key that party controls. What
remains outside that claim is denial of service — deleting or corrupting
snapshots, the manifest, or wraps — and the fact that revocation does not
re-encrypt existing objects; it bounds what a revoked device can learn from
then on, which is the threat it addresses.
...
wraps anywhere, a chain of the wrong shape, and any schema version but the
current one.
