...
| Keyring | `<prefix>/keyring.v1.json` in storage | Per key generation, the root key wrapped to each enrolled device's public key (age) and under the recovery code (argon2id → XChaCha20-Poly1305). Format version 2 binds every wrap to the profile id and generation (device wraps inside the age payload, the recovery wrap as AEAD associated data), so a wrap cannot be replayed in another keyring or generation; version 1 objects are read and rewritten as version 2 on the first write that holds the root key. Written create-only on init and with compare-and-swap on every later change. |
