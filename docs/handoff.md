...
Phase 4 `v0.4.0-rc.8` lets you continue the same task in a different coding
...
This document is the implementation contract for `v0.4.0-rc.8`. It deliberately
...
| Mode | Destination | Fidelity | Status in `v0.4.0-rc.8` |
| ---- | ----------- | -------- | ------------------ |
| **Native resume** | Same agent | Highest; vendor session semantics kept | Shipped since `v0.2.0` |
| **Structured handoff** | Different agent | Task state + selected verbatim history + evidence | **`v0.4.0-rc.8`** |
| **Reconstructed conversation** | Different agent | Visible history written into target-native storage | Not shipped in `v0.4.0-rc.8` |

## Supported directions in `v0.4.0-rc.8`
...
Gemini CLI, OpenCode, and Grok Build are **source-only** in `v0.4.0-rc.8`: you can
...
Be clear about the limit: in `v0.4.0-rc.8` this is a **prompt-level contract**.
