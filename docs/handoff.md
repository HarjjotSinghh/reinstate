...
Phase 4 `v0.4.0-rc.7` lets you continue the same task in a different coding
...
This document is the implementation contract for `v0.4.0-rc.7`. It deliberately
...
| Mode | Destination | Fidelity | Status in `v0.4.0-rc.7` |
| ---- | ----------- | -------- | ------------------ |
| **Native resume** | Same agent | Highest; vendor session semantics kept | Shipped since `v0.2.0` |
| **Structured handoff** | Different agent | Task state + selected verbatim history + evidence | **`v0.4.0-rc.7`** |
| **Reconstructed conversation** | Different agent | Visible history written into target-native storage | Not shipped in `v0.4.0-rc.7` |

## Supported directions in `v0.4.0-rc.7`
...
Gemini CLI, OpenCode, and Grok Build are **source-only** in `v0.4.0-rc.7`: you can
...
Be clear about the limit: in `v0.4.0-rc.7` this is a **prompt-level contract**.
