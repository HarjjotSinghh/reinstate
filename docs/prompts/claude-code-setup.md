...
**Prompt version:** 6
**Pinned Reinstate release:** `v0.1.0`
...
which must pin the exact release `v0.1.0`. Never substitute `latest`,
...
3. Download the selected bootstrap to a temporary file before executing it.
   ...
   - the canonical installer downloads binaries only from
     `https://github.com/HarjjotSinghh/reinstate/releases/download/v0.1.0/`.
...
5. Reconfirm the exact effective Reinstate home, then run
   `rein version --json`. Require version `0.1.0`. Run
