...
| --- | --- | --- |
| GitHub release and native Linux files | `v0.6.0-rc.6` is the current GitHub Release and public-installer pin; stable remains `v0.5.1`. Package-manager channels may still list an earlier tag until publish workflows promote this one | Keep Intel macOS and Linux/WSL2 artifacts labeled preview; do not advertise Homebrew/Scoop/`npm` as `v0.6.0-rc.6` until those listings exist |
