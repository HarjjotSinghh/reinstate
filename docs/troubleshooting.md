...
1. Run `rein version --json` and require `0.1.0-rc.8` or newer.
...
Do not manually move the session file. RC8 rejects legacy snapshots whose
Claude project identity cannot be mapped safely; reinstall RC8 on the source
device and push that selected session again to a fresh RC8 profile.
...
first. RC8 provides `init --force`, which backs up config and state together
