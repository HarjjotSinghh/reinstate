...
// TestSessionIDCollisionExitsCompatibility pins the exit code the Grok
// acceptance contract requires for a pinned-id collision.
//
// The collision is a destination-state refusal, not a Reinstate fault, and it
// is not recoverable by making room, because Reinstate never writes into a
// vendor session store. Returning it unwrapped made it exit 1, which is
// indistinguishable from a crash to any script checking the code.
func TestSessionIDCollisionExitsCompatibility(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	const id = "367ff14d-719b-42a5-b172-517410d081c7"
	dir := filepath.Join(root, "sessions", url.QueryEscape(filepath.Join(root, "ws")), id)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatalf("seed an existing session: %v", err)
	}

	target := &GrokTarget{Root: root}
	err := target.refuseExistingSessionID(id)
	if err == nil {
		t.Fatal("an existing session id was accepted")
	}
	if !errors.Is(err, ErrGrokSessionIDCollision) {
		t.Fatalf("error = %v, want ErrGrokSessionIDCollision", err)
	}
	var pe *PipelineError
	if !errors.As(err, &pe) {
		t.Fatalf("collision error carries no exit code: %v", err)
	}
	if pe.Code != exitcode.Compatibility {
		t.Fatalf("exit code = %d, want %d (compatibility)", pe.Code, exitcode.Compatibility)
	}
}
