...
// embedded-store design rests on. Any open that is not read-only and immutable
// creates -wal and -shm beside the vendor's database, which is a write under an
// agent root performed by a tool whose contract is that it never writes there.
func TestOpenCodeTargetVerifyOpensTheStoreReadOnly(t *testing.T) {
	t.Parallel()

	dsn := transcript.OpenCodeReadOnlyDSN(filepath.Join("root", transcript.OpenCodeDatabaseName))
	for _, guard := range []string{"mode=ro", "immutable=1", "query_only(1)"} {
		if !strings.Contains(dsn, guard) {
			t.Fatalf("read-only DSN %q is missing %q", dsn, guard)
		}
	}
...
// TestPipelineRefusesAPlanThatHidesItsBootstrap keeps the substitution from
// guessing. A launched prompt that differs from plan.Bootstrap would also make
// the post-launch reconciliation unable to recognize its own session.
func TestPipelineRefusesAPlanThatHidesItsBootstrap(t *testing.T) {
	t.Parallel()

	plan := DestinationPlan{
		Agent: sessionindex.AgentOpenCode,
		Args:  []string{OpenCodeNewSessionFlag, "something else entirely"},
	}
	if _, err := rewriteBootstrapArgs(plan, []byte("planned"), []byte("rendered")); err == nil {
		t.Fatal("argv that does not carry the planned bootstrap was rewritten anyway")
	}
}
