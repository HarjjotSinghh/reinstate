...
func (t *OpenCodeTarget) Verify(ctx context.Context, plan DestinationPlan, launchStart time.Time) (string, string, error) {
	...
	// vendorsqlite reads a private copy of the database and its write-ahead
	// log, so a session the vendor has only just created is visible. An
	// immutable in-place handle ignores that log by definition, which is why
	// reconciliation used to report unresolved for exactly the session it had
