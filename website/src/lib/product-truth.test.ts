...
describe('central product-truth drift guard', () => {
  ...
  it('keeps the v0.5.1 catalog line from claiming an unpublished tag or extra T5 agents', () => {
    ...
    // OpenCode is the first agent to sit below T5 with a handoff destination.
    // What still has to be pinned is that encrypted sync did not move with it.
    expect(compatibilityAgents.filter((agent) => agent.tier === 'T3')).toEqual([]);
    expect(
      compatibilityAgents.filter((agent) => agent.tier === 'T4').map((agent) => agent.key),
    ).toEqual(['opencode']);
    expect(
