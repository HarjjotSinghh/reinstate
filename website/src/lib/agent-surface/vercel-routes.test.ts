...
describe('injectAgentRoutes', () => {
  it('places negotiation before the filesystem phase and not-found right after it', () => {
    ...
    expect(routes.at(-1)).toEqual({ src: '^/.*$', dest: '_render', status: 404 });
  ...
  it('creates the filesystem phase when the adapter emitted none', () => {
    ...
    expect(result.routes!.map((route) => route.handle ?? route[AGENT_ROUTE_MARKER])).toEqual([
      'markdown',
      'filesystem',
      'not-found',
    ]);
...
describe('agent routes', () => {
  it('negotiate only GET/HEAD page paths that list text/markdown', () => {
    ...
    expect(route.dest).toBe('/agent-surface/markdown?path=$1');
  ...
  it('send non-HTML clients for unknown paths to the Markdown 404', () => {
    ...
    expect(route.dest).toBe('/agent-surface/not-found?path=$1');
    expect(route.check).toBe(true);
  ...
  it('capture the full page path for the rewrite', () => {
    const source = new RegExp(markdownNegotiationRoute().src!);
    expect('/docs/getting-started'.match(source)?.[1]).toBe('/docs/getting-started');
    expect('/'.match(source)?.[1]).toBe('/');
  });
});

describe('verifyAgentRoutes', () => {
  it('reports missing, duplicated, and misplaced routes', () => {
    ...
    const misplaced = astroConfig();
    misplaced.routes!.push(markdownNegotiationRoute(), markdownNotFoundRoute());
