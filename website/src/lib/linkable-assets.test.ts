...
describe('evidence-safe linkable assets', () => {
  ...
  it('unifies current compatibility and tagged release evidence', async () => {
    ...
    expect(agentVersionHistory).toHaveLength(30);
    expect(new Set(agentVersionHistory.map(({ source }) => source)).size).toBe(30);
    expect(
      agentVersionHistory.find(({ version }) => version === 'v0.4.0-rc.9')
        ?.rangeChange,
    ).toContain('Unchanged from v0.4.0-rc.8');
    expect(
