...
describe('published GitHub CLI release contract', () => {
  it('requires checksums, five platform archives, their SBOMs, and source', () => {
    ...
    expect(assets).toContain('reinstate_0.1.0-rc.7_source.tar.gz');
  ...
  it('reports every missing required asset and rejects duplicate names', () => {
    ...
    const missing = [
      'checksums.txt',
      'reinstate_0.1.0-rc.7_windows_amd64.zip',
      'reinstate_0.1.0-rc.7_linux_arm64.tar.gz.sbom.json',
      'reinstate_0.1.0-rc.7_source.tar.gz',
    ];
