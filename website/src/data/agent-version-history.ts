...
> = {
  'v0.6.0-rc.8': {
    ...
    compatibilityChange:
      'Corrective candidate after v0.6.0-rc.7 tagged-artifact acceptance on native Windows x64 ended device verdict FAIL (214/0/1/0 of 215 required rows); that candidate\'s own targeted fix, the widened Codex CLI range, was fully confirmed on real, completed conversational turns, and every row carried from v0.6.0-rc.6 (MatrixG:G1\'s Codex half, codex:E1-E3, grok:E1-E3, MatrixH:H7) cleared to PASS. The run found one new regression: CLI row 13 (FAIL, new finding), where the interactive switcher\'s default all-projects scope rendered a Ready or Warn session as Blocked in 11 of 15 independent launches, non-self-correcting once wrong, because a readiness check that ran out of its shared preflight budget was read exactly like a real blocking finding and then cached as final. Fixes exactly that: internal/tui/readiness.FromReport now tells a genuine finding apart from a timed-out check, an unresolved probe is retried instead of cached, and the background prober gets its own more generous timeout budget; a follow-up ensures a permanently, deterministically broken agent install still settles on Blocked rather than checking forever. No agent tier changes, no range changes. See CHANGELOG.md.',
    implementationSource:
      'https://github.com/HarjjotSinghh/reinstate/blob/v0.6.0-rc.8/internal/tui/readiness/prober.go',
  },
  'v0.6.0-rc.7': {
    rangeChange:
      'Widens the verified Codex CLI range to 0.133.0-0.153.4 (was 0.133.0-0.149.0), on native Windows evidence, under ADR 0005 D3: the acceptance host\'s Codex CLI self-updated past the in-tree 0.149.0 ceiling, so a real launch plan against it correctly refused with exit 5 (agent.version), the same drift the v0.6.0-rc.6 tagged report\'s own codex:E1-E3 rows recorded as NOT TESTED (version drift). This widening\'s evidence is read-only (codex --version/--help/exec --help/resume --help/fork --help output shape, a sanitized rein doctor --agents --json probe, and a committed fixture home exercised through rein resume --dry-run --json); version-output parsing and every launch-plan-relevant --help flag (resume, fork, the positional prompt argument) are unchanged from 0.149.0. Completed-turn codex:E1-E3 evidence against 0.153.4 was confirmed via real, completed conversational turns in the v0.6.0-rc.7 tagged run. Claude Code (2.1.219-2.1.263), Grok Build (1.0.5-1.0.13), OpenCode (1.18.21-1.18.29), and Qwen Code (0.21.12-0.23.0) ranges are unchanged. macOS evidence is pending.',
    compatibilityChange:
      'Corrective candidate after v0.6.0-rc.6 tagged-artifact acceptance on native Windows x64 ended device verdict FAIL (211/1/0/3 of 215 required rows, zero product defects; this candidate\'s own two targeted fixes, grok:E1-E3 and MatrixH:H7, both cleared to PASS). The run found MatrixG:G1\'s Codex half PARTIAL (the acceptance host\'s live, authenticated Codex account was usage-limit exhausted) and codex:E1-E3 NOT TESTED (version drift) (the host\'s Codex CLI self-updated to 0.153.4, above the verified 0.149.0 ceiling). Widens exactly the verified Codex CLI range. No agent tier changes. Its own tagged-artifact acceptance ended device verdict FAIL (214/0/1/0 of 215 required rows); the widened range was fully confirmed, and the run found a new interactive-switcher regression (CLI row 13), fixed in v0.6.0-rc.8. See CHANGELOG.md.',
    implementationSource:
      'https://github.com/HarjjotSinghh/reinstate/blob/v0.6.0-rc.7/docs/compatibility.md',
  },
  'v0.6.0-rc.6': {
    rangeChange:
  ...
  'v0.6.0-rc.1': {
    ...
    implementationSource:
      'https://github.com/HarjjotSinghh/reinstate/blob/v0.6.0-rc.1/docs/compatibility.md',
  },
  'v0.5.2-rc.1': {
    claude:
