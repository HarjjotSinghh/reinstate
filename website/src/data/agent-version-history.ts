...
> = {
  'v0.6.0-rc.7': {
    rangeChange:
      'Widens the verified Codex CLI range to 0.133.0-0.153.4 (was 0.133.0-0.149.0), on native Windows evidence, under ADR 0005 D3: the acceptance host\'s Codex CLI self-updated past the in-tree 0.149.0 ceiling, so a real launch plan against it correctly refused with exit 5 (agent.version), the same drift the v0.6.0-rc.6 tagged report\'s own codex:E1-E3 rows recorded as NOT TESTED (version drift). This widening\'s evidence is read-only (codex --version/--help/exec --help/resume --help/fork --help output shape, a sanitized rein doctor --agents --json probe, and a committed fixture home exercised through rein resume --dry-run --json); version-output parsing and every launch-plan-relevant --help flag (resume, fork, the positional prompt argument) are unchanged from 0.149.0. Completed-turn codex:E1-E3 evidence against 0.153.4 is deferred to this candidate\'s own tagged run, once the acceptance host\'s Codex account usage limit resets. Claude Code (2.1.219-2.1.263), Grok Build (1.0.5-1.0.13), OpenCode (1.18.21-1.18.29), and Qwen Code (0.21.12-0.23.0) ranges are unchanged. macOS evidence is pending.',
    compatibilityChange:
      'Corrective candidate after v0.6.0-rc.6 tagged-artifact acceptance on native Windows x64 ended device verdict FAIL (211/1/0/3 of 215 required rows, zero product defects; this candidate\'s own two targeted fixes, grok:E1-E3 and MatrixH:H7, both cleared to PASS). The run found MatrixG:G1\'s Codex half PARTIAL (the acceptance host\'s live, authenticated Codex account was usage-limit exhausted) and codex:E1-E3 NOT TESTED (version drift) (the host\'s Codex CLI self-updated to 0.153.4, above the verified 0.149.0 ceiling). Widens exactly the verified Codex CLI range. No agent tier changes. See CHANGELOG.md.',
