...
description: "Configure a private Amazon S3 bucket and least-privilege credentials for Reinstate RC8, then verify encrypted Claude Code or Codex session storage."
...
howToSteps:
  ...
  - name: "Grant only the required S3 permissions"
    text: "Attach a prefix-limited policy to a dedicated non-root IAM principal and create the access-key pair that RC8 can store in the operating-system keyring."
...
This guide connects Reinstate `v0.1.0-rc.8` to an existing Amazon S3 bucket.
...
- The S3 service endpoint and bucket are separate inputs. For a normal Region,
  ...
  `--region`.
- RC8 requires `ListBucket`, `GetObject`, `PutObject`, and `DeleteObject`
...
- RC8 accepts an access-key ID and secret-access-key pair but no AWS session
...
Attach the reviewed policy to a dedicated non-root IAM principal. RC8 does not
...
- RC8 does not request provider-side encryption headers. A bucket default such
...
Not directly in RC8. Temporary AWS credentials include a session token, while
...
relevant KMS permissions. RC8 does not send request-specific SSE headers, so a
