...
description: "Move one encrypted Claude Code or Codex session from macOS to native Windows with RC7 path remapping, dry-runs, backups, and native resume."
answer: "To move a coding-agent session from Mac to Windows with Reinstate RC7, map one canonical project ID to the Mac and Windows checkout paths, push one selected Claude Code or Codex session from the Mac, join the same encrypted storage profile on native Windows, preview the pull, restore it, and resume it with the same vendor."
...
prerequisites:
  ...
  - "Reinstate v0.1.0-rc.7 plus a tested Claude Code or Codex CLI version on both computers"
...
howToSteps:
  - name: "Verify the two native environments"
    text: "Confirm the RC7 binary, same-vendor agent version, native operating system, and separate absolute project path on each computer before configuring synchronization."
...
from macOS to **native Windows 11** through Reinstate `v0.1.0-rc.7`. It uses
...
- Resume remains **same-vendor**: Claude Code to Claude Code, or Codex CLI to
  Codex CLI. RC7 does not convert one vendor's transcript into the other
...
Current RC7 source evidence recognizes Claude Code `2.1.219`–`2.1.220` and
...
Install RC7 from **native Windows PowerShell**, not a WSL shell:
...
commit, or unreviewed dirty working tree. RC7 maps structural paths; it does
not synchronize Git state or prove environment equivalence.

**Expected result:** both `rein version --json` commands report
`0.1.0-rc.7`, the Mac and 64-bit native Windows environments are identified
...
passphrase. RC7 creates a profile UUID, writes and deletes a two-byte storage
...
unsafe compatibility override; RC7 has none.
...
- [ ] Both machines report RC7, native OS identity, and a recognized version
...
- RC7 is explicit push/pull synchronization, not background automatic sync.
...
No. RC7 restores Claude Code state for Claude Code and Codex state for Codex
...
When an existing destination may be replaced, RC7 checks whether that vendor
