...
description: "Configure a private Cloudflare R2 bucket and bucket-scoped S3 credentials for Reinstate RC6, then verify encrypted Claude Code or Codex storage."
...
prerequisites:
  ...
  - "Reinstate v0.1.0-rc.6 on a compatible device with Claude Code or Codex CLI"
...
This guide connects Reinstate `v0.1.0-rc.6` to an existing Cloudflare R2
...
- Session resume remains **same-vendor**: Claude Code to Claude Code and Codex
  CLI to Codex CLI.
- RC6 is a release candidate. This guide is not evidence that the outstanding
...
RC6 accepts the generated access-key ID and secret access key. It does not
...
**Expected result:** the pinned installer reports `v0.1.0-rc.6`. Before
...
This personal check does not claim physical RC6 acceptance.
...
| --- | --- | --- |
| `storage probe put failed` (exit `4`) | Wrong endpoint, bucket, credential, jurisdiction, or a read-only token. | Compare the token confirmation endpoint and selected bucket; require Object Read & Write on that bucket only. |
| `storage probe cleanup failed` (exit `4`) | Delete is denied or a bucket lock covers the generated probe prefix. | Preserve the error, remove the stray probe through approved R2 tooling, and fix the token or lock before retrying. |
| `SignatureDoesNotMatch` or authorization failure | The account ID, jurisdiction endpoint, access-key pair, or Region is wrong. | Use the exact R2 S3 endpoint, `--region auto`, and the credential generated for the selected bucket. |
| `config missing` (exit `3`) after failed init | RC6 intentionally did not save local config because the remote probe failed. | Fix R2 first and rerun the same reviewed `rein init`. |
...
| `remote profile manifest not found` on another device | Profile UUID, bucket, prefix, endpoint, or jurisdiction differs from the first device. | Reuse every non-secret storage coordinate and the exact first-device `profile_id`; do not create an empty manifest. |
| Conflict exit `6` | The local and remote copies diverged after sync. | Preserve the conflict record and both histories; resolve explicitly instead of deleting R2 objects. |

## Safe rollback and undo

Reinstate RC6 does not provide a general `rein undo` or remote-session delete
command.
...
- RC6 accepts the access-key pair generated for an R2 API token but no
  additional session-token field.
- Dashboard token scope is bucket-level. RC6 generates the first profile UUID
...
- R2's S3 compatibility evolves. This guide depends only on the current
  Put/Get/Head/Delete/List and conditional-write behavior used by RC6.
