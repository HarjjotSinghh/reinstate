...
description: "Use every current Reinstate RC7 command with its purpose, parameters, expected evidence, platform notes, failure modes, and safe recovery path."
order: 14
...
every command shipped by Reinstate `v0.1.0-rc.7`, including what it does, what
success looks like, the flags it accepts, platform-specific behavior, common
failures, and the available recovery path.

> **Current scope:** RC7 supports same-vendor Claude Code and Codex session
...
- Install and verify `v0.1.0-rc.7` before relying on this syntax.
...
integration surface, but RC7 remains pre-1.0 and may change before v1.0.
...
contains `version`, `commit`, and `date`; the public RC7 installer must report
`0.1.0-rc.7`.
...
**Parameters:** `--json` selects structured output. RC7 status has no agent or
session filter.
...
**Undo or recovery:** dry-run needs no undo. Snapshots are immutable and RC7
...
- `rein version --json` showing `0.1.0-rc.7`;
