...
description: "Use every current Reinstate RC8 command with its purpose, parameters, expected evidence, platform notes, failure modes, and safe recovery path."
order: 14
...
> **Current scope:** RC8 supports same-vendor Claude Code and Codex session
...
- Install and verify `v0.1.0-rc.8` before relying on this syntax.
...
integration surface, but RC8 remains pre-1.0 and may change before v1.0.
...
contains `version`, `commit`, and `date`; the public RC8 installer must report
`0.1.0-rc.8`.
...
**Parameters:** `--json` selects structured output. RC8 status has no agent or
session filter.
...
**Undo or recovery:** dry-run needs no undo. Snapshots are immutable and RC8
...
- `rein version --json` showing `0.1.0-rc.8`;
