...
> cross-harness settings are roadmap work and do not exist in RC7.

## Prerequisites

- Reinstate `v0.1.0-rc.7`, verified with `rein version --json`.
...
scope. RC7 requires the existing encrypted `manifest.age` to be present and
...
RC7 refuses ordinary `init` when `config.toml` or `state.json` already exists
