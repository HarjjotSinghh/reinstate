...
Reinstate `v0.1.0-rc.7` is a pre-1.0 release candidate; Claude Code and Codex
...
directory key expected on this device. A snapshot made before RC7 may also lack
...
Require Reinstate `0.1.0-rc.7` or newer on both devices. Correct the destination
mapping for the existing canonical project ID, close Claude Code, and repeat
the scoped dry-run. If RC7 rejects a legacy snapshot whose Claude project
identity cannot be mapped, install RC7 on the source device and push that one
session again to an intentionally fresh RC7 profile. Do not manually move the
...
File an issue when RC7 or newer plans and writes the correct destination,
...
can read that object, and RC7 still reports it missing. Include redacted
