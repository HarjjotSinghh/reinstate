...
`v0.6.0-rc.4` release candidate — Reinstate Hop (cloud continuity) and the
...
x64. Intel macOS and Linux/WSL2 stay preview/unverified. The `v0.6.0-rc.4`
...
| `rein login`, `rein whoami` | 0.6.0-rc.4 (candidate) | no |
| `rein init --hop`, `rein account init\|recover\|join\|status` | 0.6.0-rc.4 (candidate) | yes |
| `rein devices [approve\|revoke]`, `rein hop status\|credentials` | 0.6.0-rc.4 (candidate) | yes |
| `rein sync verify`, `rein sync migrate --to byo` | 0.6.0-rc.4 (candidate) | yes |
| `rein daemon run\|install\|start\|stop\|uninstall\|status` | 0.6.0-rc.4 (candidate) | yes |
| bare `rein` interactive switcher, `ctrl+k` palette, `--plain` | 0.6.0-rc.4 (candidate) | no |

Flag-level syntax lives in the [CLI reference](/docs/cli-reference). Rows
marked "candidate" ship in `v0.6.0-rc.4`, not in stable `v0.5.1`.
...
## v0.6.0-rc.4 — Reinstate Hop and the interactive CLI (release candidate)

`v0.6.0-rc.4` is a release candidate, not a stable claim: stable remains
