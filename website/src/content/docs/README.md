# Documentation

**CLI:** prefer short alias **`rein`**. Full name **`reinstate`** is equivalent.

| Document | Description |
| -------- | ----------- |
| [Getting started](getting-started.md) | Install and dual-device setup |
| [Phase 1 Mac + Windows acceptance](https://github.com/HarjjotSinghh/reinstate/blob/main/docs/testing/phase-1-mac-windows-acceptance.md) | Strict two-device release checklist |
| [Architecture](architecture.md) | System design |
| [Adapters](adapters.md) | Agent support matrix |
| [Cross-agent continuation](cross-agent-continuation.md) | Core quota/outage handoff and fidelity model |
| [Universal agent configuration](universal-configuration.md) | Planned MCP/skills/loops/plugins/settings portability |
| [Security model](security-model.md) | Threat model and defaults |
| [Comparison](comparison.md) | vs alternatives |
| [FAQ](faq.md) | Common questions |
| [Troubleshooting](troubleshooting.md) | Debug guide |

Project-level docs live in the repository root:

- [README](https://github.com/HarjjotSinghh/reinstate/blob/main/README.md)
- [ROADMAP](https://github.com/HarjjotSinghh/reinstate/blob/main/ROADMAP.md)
- [CONTRIBUTING](https://github.com/HarjjotSinghh/reinstate/blob/main/CONTRIBUTING.md)
- [SECURITY](https://github.com/HarjjotSinghh/reinstate/blob/main/SECURITY.md)
- [SUPPORT](https://github.com/HarjjotSinghh/reinstate/blob/main/SUPPORT.md)
